import { Phone, MessageCircle, Mail, Globe, Download, MapPin, CheckCircle2, Sparkles, CheckCircle, Sun, Moon } from 'lucide-react';
import { SOCIAL_ICONS } from '@/lib/useBusinessCardLogic';
import type { TemplateProps } from './types';

function BannerTemplate({ profile, logic }: TemplateProps) {
  const {
    saved,
    setServicesOpen,
    dark,
    ...
    <div className="w-full max-w-[400px] h-full max-h-[820px] flex flex-col animate-fade-in-up">
      <div className={`rounded-3xl overflow-hidden card-shadow flex flex-col flex-1 min-h-0 ${darkCard}`}>
        <div className="relative shrink-0">
          <div className="h-24 relative brand-gradient">
            <button
              onClick={() => setDark(!dark)}
              className="absolute top-2.5 right-2.5 w-7 h-7 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center hover:bg-white/30"
            >
          ...
          <div className="flex justify-center -mt-10">
            <div className="relative animate-scale-in">
              ...
              <div className="absolute -bottom-0.5 -right-0.5 w-6 h-6 rounded-full bg-[var(--brand)] border-2 border-white flex items-center justify-center">
        ...
        {contactCount > 0 && (
          <div className="px-3 pt-2 pb-1 shrink-0">
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <h3 className={`text-[10px] font-bold tracking-wide uppercase ${darkLabel}`}>Contact direct</h3>
            </div>
        ...
        <div className="px-3 pt-2 pb-3 shrink-0 space-y-2 flex-1 flex flex-col justify-end">
          <button
      ...
      <div className="mt-2 px-2 text-center shrink-0">
